...
router.post('/check', async (req, res) => {
  ...
  if (await isTokenDead(hash)) {
    return res.json({ status: 'TOKEN_DEAD' })
  }
  ...
  if (!socket) {
    return res.json({ status: 'PANEL_DEAD' })
  }
  ...
  if (handshakeResult.status === 'TIMEOUT') {
    return res.json({ status: 'PANEL_DEAD' })
  }
...
router.post('/pair', async (req, res) => {
  ...
  try {
    ...
    console.error('Pairing proxy error:', e.message)
    return res.status(502).json({ error: 'Pairing service unavailable' })
  }
})

// GET /dashboard/code/:sessionId
